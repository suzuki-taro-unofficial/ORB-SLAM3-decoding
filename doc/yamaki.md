...
### 元の実装
1. Runで`NewDetectCommonRegions`を呼び出し、キーフレームの取り出し、スキップ条件の確認、ループおよびマージの検知を行う
2. マージが検出されたら、マージの処理を行う。
3. マージが検出されず、ループが検出されたら、ループとじ込みを行う。

### 変更後の実装
1. Runでキーフレーム取り出す
2. Runでスキップ条件を確認する
3. マージの検出を行う
4. マージが検出されたら、マージの処理を行う。
5. マージが検出されなかったら、ループの検出を行い、検出されたら、ループとじ込みを行う。
...
- 並列にすることによる変更点は理屈をつける。
- モナドが出る前の関数型言語の並列処理を参考にするといいかもしれない。
- なぜコピーをたくさんしているのかを見直した方がいいかもしれない。FRP4章やfrpotのコードを見直してみる。
- data構造に対してどういうアクセスがあるのかを洗い出す。
